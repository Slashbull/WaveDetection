...
def load_and_validate_data() -> Tuple[pd.DataFrame, Dict[str, any]]:
    ...
    try:
        ...
        critical_cols = [
            ...
            'market_cap', 'eps_current', 'eps_last_qtr', 'eps_change_pct', 'pe' # Added for fundamentals
        ]
        ...
        for col in ['ret_1d', 'ret_3d', 'ret_7d', 'ret_30d', 'ret_3m', 'ret_6m', 'ret_1y', 'ret_3y', 'ret_5y',
                    'rvol', 'volume_acceleration', 'from_low_pct', 'from_high_pct', 'eps_change_pct', 'pe']:
            if col in df.columns:
...
def render_sector_leaderboard(df: pd.DataFrame, sector_metrics_for_ui: pd.DataFrame):
    ...
    # Create treemap for visual overview
    fig_treemap = px.treemap(
        sector_metrics,
        path=[px.Constant("All Sectors"), 'sector'], # Add a constant root for better hierarchy
        values='Total Stocks',
        color='Avg EDGE Score',
        hover_data={'Avg EDGE Score': ':.1f', 'Total Stocks': True, 'SUPER EDGE Count': True},
    ...
    fig_treemap.update_layout(height=500, margin=dict(t=50, l=10, r=10, b=10))
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if 'tag' in df_signals.columns:
            ...
            if not action_items.empty:
                action_cols = [
                    ...
                    'vol_score', 'mom_score', 'rr_score', 'fund_score' # Include all scores
                ]
        ...
        if not df_signals.empty:
            signal_cols = [
                ...
                'eps_current', 'eps_last_qtr', 'eps_change_pct', 'pe', 'quality_consolidation', 'momentum_aligned' # Include fundamental & special indicators
            ]
...
def render_ui():
        ...
        if not current_display_df.empty:
            ...
            with col_export_csv:
                ...
                st.download_button(
                    ...
                    csv_export,
                    f"edge_signals_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    "text/csv",
                    type="primary"
                )
            with col_export_excel:
                ...
                st.download_button(
                    ...
                    excel_buffer,
                    f"EDGE_Report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
    ...
    with tabs[1]:
        ...
        if not super_df.empty:
            ...
            for idx, (_, row) in enumerate(super_df.iterrows()):
                ...
                with st.expander(
                    ...
                    with col_det3:
                        ...
                        st.markdown(f"**Volume Pattern:** {row.get('volume_classification', 'N/A')}")
                        ...
                        if row.get('pattern_analysis') and row['pattern_analysis'].get('top_pattern'):
            ...
            if not explosive_df.empty:
                ...
                cols_exp = ['ticker', 'company_name', 'EDGE', 'price', 'volume_classification', 'decision']
    ...
    with tabs[6]:
        ...
        with col3:
            st.metric("Data Timestamp", diagnostics.get('timestamp', datetime.datetime.now()).strftime('%Y-%m-%d %H:%M'))
        ...
        critical_cols_display = [
            ...
            'market_cap', 'eps_current', 'eps_last_qtr', 'eps_change_pct', 'pe',
        ...
        with col_exp1:
            ...
            st.download_button(
                ...
                csv_full,
                f"edge_protocol_full_processed_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                "text/csv"
            )
        ...
        with col_exp2:
            ...
            if not high_edge_df.empty:
                ...
                st.download_button(
                    ...
                    csv_high,
                    f"edge_high_signals_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    "text/csv"
                )
